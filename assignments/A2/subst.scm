...
(define remove_paren_helper (lambda(elem)
    (cond 
      ((symbol? elem) elem)
      ((not (= (length elem) 1)) (map remove_paren_helper elem))
      ((= (length elem) 1) (remove_paren_helper (car elem)))
    )
))
...
;;(define que1 (list 'fun 'x 'fun 'z 'x (list 'fun 'y 'y 'y) 'z 'z))
;;;(define que1 (list 'fun 'z 'x (list 'fun 'y 'y 'y) 'z 'z))
;;(define ans1 (make-abs 'x (make-abs 'z (make-app (make-app (make-app 'x (make-abs 'y (make-app 'y 'y))) 'z) 'z))))
;;(define que2 (list 'fun 'x (list (list 'x (list (list (list 'x)))))))
;;(define ans2 (list 'fun 'x (list 'x 'x)))
;;(define que3 (list (list (list 'x))))
;;(define ans3 'x)
;;(define que4 (list (list 'x 'y (make-abs 'z 'x)) 'y 'x))
;;(define ans4 '((((x y) (fun z x)) y) x))
;;
;;(equal? (parse-lambda que1) ans1)
;;(equal? (parse-lambda que2) ans2)
;;(equal? (parse-lambda que3) ans3)
;;(equal? (parse-lambda que4) ans4)
;;(parse-lambda '((fun m fun n fun f m (n f)) (fun f fun x f (f x)) (fun f fun x f (f (x)))))
;;(interpret '((fun m fun n fun f m (n f)) (fun f fun x f (f x)) (fun f fun x f (f (x)))))
;;(parse-lambda '((fun m fun n fun f m (n f)) (fun f fun x f (f x)) (fun f fun x f (f (x)))))
;;(interpret '((fun m fun n fun f m (n f)) (fun f fun x f (f x)) (fun f fun x f (f (x)))))
;;(parse-lambda '((fun x fun y fun z x y z) a b c))
;;(interpret '((fun x fun y fun z x y z) a b c))
;;
;;(define L (parse-lambda '((fun m ((fun n (fun f m (n f))) (fun f fun x f (f (f x))))) (fun f fun x f (f x)))))
;;(parse-lambda L)
;;(interpret L)
;;
;;(interpret '((fun m fun n fun f n m f) (fun f fun x f (f (f x))) (fun f fun x f (f (x)))))
;;;;(parse-lambda '((fun m ((fun n (fun f m (n f))) (fun f fun x f (f x)))) (fun f fun x f (f (x)))))
;;;;(interpret '((fun m ((fun n (fun f m (n f))) (fun f fun x f (f x)))) (fun f fun x f (f (x)))))
;;(interpret '((fun m ((fun n (fun f n m f)) (fun f fun x f (f (f x))))) (fun f fun x f (f x))))
