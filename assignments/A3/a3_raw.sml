...
(* End of Part B, Part D *)


(*
val res = W initenv (Let ("f", Abs("x", Var "x"), If(App(Var "f", Bool true), App(Var "f", Int 1), Int 0)))
...
Letex("divzero",
   Let ("safediv", Abs("x", Abs("y",
           If (App (App (Prim Eq, Var "y"), Int 0),
               Raise(Let("x", Abs("c", Var "divzero"), App(Var "x", Bool true))),
               App (App (Prim Div, Var "x"), Var "y")
			  )
		 )),
       Handle(Let("x", Abs("c", Var "divzero"), App(Var "x", Bool true)),
...
W initenv (Let ("y",Abs("x", Var("x")), Var "y"));
val res3 =
W initenv (
	App(Abs("x", Var "x"), Bool true)
)

val res4 =
W initenv (
	Let("x", Abs("c", Int 1), App(Var "x", Bool true))
)

val res5 = 
W initenv (
	Letex("err", (Let("x", Abs("c", Var "err"), App(Var "x", Bool true))))
)
